...
function tla_ads() {
	...
	$LOCAL_XML_FILENAME = "local_312262.xml";
	...
	if( filemtime($LOCAL_XML_FILENAME) < (time() - 3600) || filesize($LOCAL_XML_FILENAME) < 20) {
		...
		tla_updateLocalXML("http://www.text-link-ads.com/xml.php?inventory_key=T5RNJOB2FUPM9K2OKIV2&referer=" . urlencode($request_uri) .  "&user_agent=" . urlencode($user_agent), $LOCAL_XML_FILENAME, $CONNECTION_TIMEOUT);
	}
	...
	if ( is_array($arr_xml) ) {
		echo "\n<ul style=\"list-style: none; margin: 0; width: 100%; padding: 0;\">\n";
		for ($i = 0; $i < count($arr_xml['URL']); $i++) {
			if( isset($arr_xml['PostID'][$i]) && $arr_xml['PostID'][$i] > 0 ) continue;
			echo "<li style=\"margin: 0; width: 13%; display: block; clear: none; float: left; padding: 0;\">".$arr_xml['BeforeText'][$i]." <a href=\"".$arr_xml['URL'][$i]."\">".$arr_xml['Text'][$i]."</a> ".$arr_xml['AfterText'][$i]."</li>\n";
		}
		echo "</ul>";
	}
